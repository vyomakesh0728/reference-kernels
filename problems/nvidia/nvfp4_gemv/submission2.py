...
// ============================================================================
// Naive full GEMV kernel: compute all output rows without tensor cores
//
// This kernel computes a block‑scaled GEMV using a straightforward dot
// product for each row.  It decodes FP4 inputs, applies FP8 scale factors
// (one scale per 16 FP4 values) and accumulates in FP32.  Each block
// handles a single row for a given batch (grid.x = M, grid.y = L).  Threads
// within the block cooperatively accumulate partial sums over the K
// dimension and then reduce them to produce the final result for that row.
// The N dimension is always 1 (GEMV), so we only use the first row of B
// and the first row of the scale tensor SFB.  This kernel uses shared
// memory to store intermediate warp sums and performs warp‑level and
// block‑level reductions using shuffles.  It is intended for correctness
// rather than peak performance but should still scale reasonably on modern
// GPUs.
//
// Arguments:
//   A_packed: pointer to packed FP4 input matrix A [L, M, K/2]
//   B_packed: pointer to packed FP4 input vector B [L, 128, K/2]
//   SFA_packed: pointer to unblocked FP8 scale factors for A [L, M, K/16]
//   SFB_packed: pointer to unblocked FP8 scale factors for B [L, 128, K/16]
//   D: pointer to output matrix [L, M]
//   M, K, L: problem dimensions
//
// Grid:
//   dim3(gridDim.x, gridDim.y) = (M, L)
// Block:
//   blockDim.x = threads per block (power of two, e.g. 256)
// Shared:
//   dynamic or static array of floats sized (#warps) for reduction
// ============================================================================
extern "C" __global__ void fp4_gemv_naive_full(
    const uint8_t* __restrict__ A_packed,
    const uint8_t* __restrict__ B_packed,
    const uint8_t* __restrict__ SFA_packed,
    const uint8_t* __restrict__ SFB_packed,
    ...
    // Guard against out‑of‑bounds grid dimensions
    if (batch >= L || row >= M) {
        return;
    }
    ...
    const int K_packed = K >> 1;   // K/2
    const int K_scales = K >> 4;   // K/16

    // Pointers into batch‑specific regions of the inputs
    const uint8_t* A_batch  = A_packed  + batch * M * K_packed;
    const uint8_t* B_batch  = B_packed  + batch * 128 * K_packed;
    const uint8_t* SFA_batch = SFA_packed + batch * M * K_scales;
    const uint8_t* SFB_batch = SFB_packed + batch * 128 * K_scales;
    ...
    // Each thread processes a strided set of k indices
    for (int k_idx = tid; k_idx < K; k_idx += block_size) {
        const int col_packed = k_idx >> 1;        // k_idx / 2
        const bool high_nib = ((k_idx & 1) == 0);

        // Decode A[row, k_idx]
        uint8_t packedA = 0;
        if (col_packed < K_packed) {
            packedA = A_batch[row * K_packed + col_packed];
        }
        uint8_t nibbleA = high_nib ? ((packedA >> 4) & 0x0F) : (packedA & 0x0F);
        float a_fp4 = __half2float(decode_fp4_e2m1(nibbleA));
        // Scale factor index: one per 16 FP4 values
        const int scale_idx = k_idx >> 4;
        float scaleA = 1.0f;
        if (scale_idx < K_scales) {
            uint8_t scale_byte = SFA_batch[row * K_scales + scale_idx];
            scaleA = decode_fp8_e4m3(scale_byte);
        }
        const float a_val = a_fp4 * scaleA;

        // Decode B[0, k_idx] (only row 0 matters for GEMV)
        uint8_t packedB = 0;
        if (col_packed < K_packed) {
            packedB = B_batch[0 * K_packed + col_packed];
        }
        uint8_t nibbleB = high_nib ? ((packedB >> 4) & 0x0F) : (packedB & 0x0F);
        float b_fp4 = __half2float(decode_fp4_e2m1(nibbleB));
        float scaleB = 1.0f;
        if (scale_idx < K_scales) {
            uint8_t scale_byte_b = SFB_batch[0 * K_scales + scale_idx];
            scaleB = decode_fp8_e4m3(scale_byte_b);
        }
        const float b_val = b_fp4 * scaleB;

        // Accumulate
        acc += a_val * b_val;
    }

    // Warp‑level reduction: each warp reduces its lanes' partial sums
    // Use a full mask as all lanes are active
    for (int offset = warpSize / 2; offset > 0; offset >>= 1) {
        acc += __shfl_down_sync(0xffffffff, acc, offset);
    }

    // Allocate shared memory for per‑warp reductions.  The number of warps
    // per block is block_size / warpSize.  We use dynamic shared memory
    // sized accordingly when launching the kernel from host.
    extern __shared__ float warp_sums[];
    const int warp_id = tid >> 5; // warp index
    const int lane_id = tid & 31; // lane index
    if (lane_id == 0) {
        warp_sums[warp_id] = acc;
    }
    __syncthreads();

    // First warp performs a reduction over the warp sums
    ...
    if (warp_id == 0) {
        ...
        if (tid == 0) {
            D_batch[row] = __float2half(block_sum);
        }
    }
}
// ============================================================================
...
void launch_fp4_gemv_optimized(
    ...
    // --------------------------------------------------------------------
    // NOTE: To ensure correctness while debugging the tensor core kernel, we
    // replace the PTX MMA launch with a naive full GEMV kernel.  The naive
    // kernel decodes FP4 inputs, applies FP8 scaling, and computes a dot
    // product for each row.  Although it is slower than the tensor core
    // implementation, it is simple and matches the reference.  Once the
    // tensor core kernel is fully verified, you can revert this call.

    const int block_size = 256;
    // Launch one block per row (grid.x = M) and one grid.y per batch (L)
    dim3 grid_naive(M, L);
    dim3 block_naive(block_size);
    // Shared memory size: one float per warp for reduction
    size_t shmem_size = ((block_size + 31) / 32) * sizeof(float);

    fp4_gemv_naive_full<<<grid_naive, block_naive, shmem_size>>>(
