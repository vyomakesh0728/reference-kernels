...
template<int TileM, int TileK, int Threads, int sfa_stride>
__device__ __forceinline__ void process_tile(
    int k_tile, int stage, int tile_rows,
    uint8_t** a_packed_stage, uint8_t** sfa_stage,
    half* b_vec_smem, half* a_f16_smem, half* b_tile_smem,
    const int M, const int K, const int K_scales_padded,
    const int tid, const int warp_id, const int lane_id,
    const bool is_producer, const bool is_consumer,
    float& c_frag_0, float& c_frag_1, float& c_frag_2, float& c_frag_3
) {
    constexpr int TileKPacked = TileK / 2;
...
fp4_gemv_rank2_cta(
    ...
    constexpr int SfaBoxK = 16;  // Rank-2: SWIZZLE_NONE
    ...
    int sfa_stage_stride = TileM * 16;  // SfaBoxK for Rank-2
    for (int s = 0; s < StageCount; ++s) {
    ...
    for (int k_tile = 0; k_tile < K; k_tile += TileK) {
        ...
        process_tile<TileM, TileK, Threads, 16>(
            k_tile, stage, tile_rows,
            a_packed_stage, sfa_stage,
            b_vec_smem, a_f16_smem, b_tile_smem,
            M, K, K_scales_padded,
            tid, warp_id, lane_id,
            is_producer, is_consumer,
            c_frag_0, c_frag_1, c_frag_2, c_frag_3
        );
...
fp4_gemv_rank3_cluster( // <- kernel start here
    ...
    constexpr int TileKPacked = TileK / 2;
    constexpr int TileScaleCount = TileK / 16;
    constexpr int SfaBoxK = 128;  // Rank-3: SWIZZLE_128B
    constexpr int StageCount = 3;
    constexpr int a_stride = TileK + 8;
    constexpr int ProducerThreads = 64;
    ...
    // For rank-3 (L>1): allocate TileM * 128 (matching TMA box_k)
    // int sfa_stage_stride = (L == 1) ? (TileM * 16) : (TileM * 128);
    int sfa_stage_stride = TileM * 128;  // SfaBoxK for Rank-3
    ...
    for (int k_tile = 0; k_tile < K; k_tile += TileK) { // for loop start
        ...
        process_tile<TileM, TileK, Threads, 128>(
            k_tile, stage, tile_rows,
            a_packed_stage, sfa_stage,
            b_vec_smem, a_f16_smem, b_tile_smem,
            M, K, K_scales_padded,
...
void launch_fp4_gemv_optimized(
    ...
    if (L == 1) {
        ...
        block = dim3(kThreads);
        // launch_config not used for L=1 anymore
    } else {
        // Rank-3: Cluster launch with cta_group::2
        cluster = dim3(2, 1, 1);
    ...
    if (L == 1) {
        // Rank-2: Use standard cudaLaunchKernel to avoid any cluster misconfiguration
        cudaError_t launch_err = cudaLaunchKernel(kernel_ptr, grid, block, kernel_args, shared_bytes, 0);
        if (launch_err != cudaSuccess) {
            throw std::runtime_error(std::string("cudaLaunchKernel failed: ") + cudaGetErrorString(launch_err));
        }
    } else {
        // Rank-3: Use cudaLaunchKernelExC for cluster launch
