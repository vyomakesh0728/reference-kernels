...
// Common decode + MMA + writeback logic shared by both rank-2 and rank-3 kernels
template<int TileM, int TileK, int Threads, int StageCount, int a_stride>
__device__ __forceinline__ void fp4_gemv_core_logic(
    uint8_t** a_packed_stage,
    uint8_t** sfa_stage,
    uint64_t* mbar_a,
    half* b_vec_smem,
    half* a_f16_smem,
    ...
    const int m_tile, const int tile_rows,
    const int tid, const int warp_id, const int lane_id,
    const bool is_producer, const bool is_consumer,
    uint32_t* stage_phase_smem,
    const bool use_tma_a,
    const int sfa_stage_stride
) {
    ...
    for (int k_tile = 0; k_tile < K; k_tile += TileK) {
        ...
        // Wait for data (caller handles TMA waits and sync)
        // This function only does decode + compute + writeback
            ...
            for (int idx = tid; idx < tile_rows * curr_cols; idx += Threads) {
                ...
                if (row < tile_rows) {
                    ...
                    if (scale_col < scale_count) {
                        ...
                        int sfa_offset = (L == 1) ? 0 : (sfa_base_scale % 128);
                        int sfa_idx = row * sfa_stage_stride + sfa_offset + scale_col;
                        int sfa_size = TileM * sfa_stage_stride;
...
#pragma unroll
        ...
        if (is_consumer && (warp_id - 2) < active_warps) {
            ...
            for (int kk = 0; kk < curr_k; kk += 16) {
                ...
                int matrix_id = lane_id / 8;
                int row_in_matrix = lane_id % 8;
        ...
        // Caller handles sync between tiles
        __syncthreads();
...
// RANK-2 kernel: CTA-only launch (L == 1)
...
fp4_gemv_rank2_cta(
    ...
    constexpr int SfaBoxK = 16;  // Rank-2: SWIZZLE_NONE, small box
    ...
    if (tid == 0 && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
        DEBUG_PRINT_ERROR("KERNEL STARTING (RANK-2 CTA): grid=(%d,%d,%d) blockDim.x=%d M=%d K=%d L=%d\n",
    ...
    const int sfb_chunks = ((K >> 4) + (kTMABoxLimit - 1)) / kTMABoxLimit;

    uint32_t total_b_bytes = static_cast<uint32_t>(K_packed);
    uint32_t total_sfb_bytes = static_cast<uint32_t>(K >> 4);
    ...
    // Prefetch first tiles using TMA (rank-2 CTA path)
    ...
    for (int k_tile = 0; k_tile < K; k_tile += TileK) {
        ...
        // Call core logic
        fp4_gemv_core_logic<TileM, TileK, Threads, StageCount, a_stride>(
            a_packed_stage, sfa_stage, mbar_a, b_vec_smem, a_f16_smem, b_tile_smem, D_batch,
            M, K, L, K_scales_padded, m_tile, tile_rows,
            tid, warp_id, lane_id, is_producer, is_consumer,
            stage_phase_smem, use_tma_a, SfaBoxK
        );
    }
#endif
}

// RANK-3 kernel: Cluster launch (L > 1)
template<int TileM, int TileK, int Threads>
__global__ void __launch_bounds__(Threads)
fp4_gemv_rank3_cluster(
    const uint8_t* __restrict__ A_packed,
    const uint8_t* __restrict__ B_packed,
    const uint8_t* __restrict__ SFA_packed,
    const uint8_t* __restrict__ SFB_packed,
    ...
    constexpr int TileKPacked = TileK / 2;
    constexpr int TileScaleCount = TileK / 16;
    constexpr int SfaBoxK = 128;  // Rank-3: SWIZZLE_128B, large box
    constexpr int StageCount = 3;
    constexpr int a_stride = TileK + 8;
    constexpr int ProducerThreads = 64;
...
#ifndef NDEBUG
    if (tid == 0 && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
        DEBUG_PRINT_ERROR("KERNEL STARTING (RANK-3 CLUSTER): grid=(%d,%d,%d) blockDim.x=%d M=%d K=%d L=%d\n",
                          gridDim.x, gridDim.y, gridDim.z, blockDim.x, M, K, L);
    }
#endif
    ...
    // Prefetch first tiles using TMA (rank-3 cluster path)
    prefetch_tile<TileM, TileK>(
        0, 0,
        use_tma_a, is_producer, warp_id, lane_id,
        m_tile, K_packed, K_scales_padded, M, L, batch,
...
void launch_fp4_gemv_optimized(
    ...
    if (tma_ok) {
        ...
        if (!map_SFB_aligned) throw std::        if (!map_SFB_aligned) throw std::runtime_error("Failed to align map_SFB");
