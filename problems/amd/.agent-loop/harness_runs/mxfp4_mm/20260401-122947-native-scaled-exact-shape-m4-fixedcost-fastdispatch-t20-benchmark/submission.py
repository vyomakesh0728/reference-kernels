...
# Candidate Card
# shape: exact m4
# regime_tag: exact_m4_fixedcost_fastdispatch_t20
# deleted_cost_center: Python-side exact m4 dispatch/tracing overhead and redundant tensor canonicalization on the tiny-shape direct-entry path
# expected_upside_source: the current t18 profile shows exact m4 compute body is already small while end-to-end latency remains fixed-cost sensitive; deleting host-side exact-m4 overhead is the next legal queue-aligned lane
# why_larger_than_noise: this branch removes per-call non-kernel work from the exact m4 hot path without changing MFMA bodies, A-pack ownership law, or B-scale contracts
# touched_symbols_or_regions: custom_kernel exact_m4 dispatch; exact m4 direct-entry argument canonicalization
# forbidden_edits: do not touch m8/m16/m32/m64/m256 paths; do not reopen A-pack ownership rewrites; keep exact MFMA kernels and B contracts unchanged
# success_gate: exact m4 mean improves from 10.0 us while preserving correctness and portfolio geomean
...
CONFIG = {
    "variant_name": "native_scaled_exact_shape_m4_fixedcost_fastdispatch_t20",
    "family": "hip_explore",
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if m == 4 and (k % 128) == 0 and (n % 16) == 0:
        ...
        if k == 512 and n == 2880:
            mod.mxfp4_mm_hip_mfma_scale_exact_m4_direct_entry_public_k512(a_in, b_q_u8, b_scale_u8, c)
        else:
