...
# regime_tag: exact_portfolio_wavepack_directentry_shufflefix
# deleted_cost_center: shared scalar A-pack helper work plus the last Python-owned exact m64 prep/orchestration path
# expected_upside_source: the exact-shape family still pays the same A-pack helper on every hot path; a wave-cooperative helper removes most scalar pack work, and compiled direct-entry deletes Python temp shaping/inflight retention on m64
# why_larger_than_noise: this keeps the same portfolio-wide deletion branch alive while removing the highest-probability undefined subgroup exchange in the new helper
# touched_symbols_or_regions: mxfp4_pack_a_fixed kernel and launcher; exact m64 direct-entry wrapper; custom_kernel exact_m64 dispatch
# forbidden_edits: do not reopen fused-A compute-path quantization; do not add LDS staging to A-pack; keep the existing exact MFMA bodies and B contracts intact
# success_gate: geomean materially improves from the 21.2289 us kept trunk
...
CONFIG = {
    "variant_name": "native_scaled_exact_shape_wavepack_directentry_t18",
    "family": "hip_explore",
