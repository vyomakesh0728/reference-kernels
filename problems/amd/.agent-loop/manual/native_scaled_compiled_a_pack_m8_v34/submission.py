...
CONFIG = {
    "variant_name": "native_scaled_compiled_a_pack_m8_v34",
    "family": "hip_explore",
...
_B_MFMA_PRESHUFFLE_CACHE: dict[tuple[object, ...], torch.Tensor] = {}
...
def _module():
    ...
    if _MODULE is None:
        ...
        _MODULE = load_inline(
            ...
            functions=["mxfp4_mm_hip", "mxfp4_mm_hip_mfma_medium", "mxfp4_mm_hip_mfma_scale_exact_m16", "mxfp4_mm_hip_mfma_scale_exact_m32", "mxfp4_pack_a_fixed"],
...
def _get_b_contract_mfma_fp4_live(
    ...
    key = ("b_contract_live", _tensor_cache_key(b_q), _tensor_cache_key(b_scale_sh))
    cached = _B_MFMA_FP4_CACHE.get(key)
    if cached is not None:
        return cached

    b_q_u8 = b_q.contiguous().view(torch.uint8)
    b_packed = _repack_mxfp4_nk_to_kn_packed(b_q_u8)
    b_scale_u8 = b_scale_sh.contiguous().view(torch.uint8)
    b_scale_rowmajor = _e8m0_unshuffle(
        b_scale_u8,
        rows=b_q_u8.shape[0],
        cols=(b_q_u8.shape[1] * 2) // SCALE_GROUP,
    )
    if len(_B_MFMA_FP4_CACHE) >= 4:
        _B_MFMA_FP4_CACHE.clear()
    _B_MFMA_FP4_CACHE[key] = (b_packed, b_scale_rowmajor)
    return b_packed, b_scale_rowmajor


def _get_b_preshuffled_mfma_fp4(b_q: torch.Tensor) -> torch.Tensor:
    key = ("b_mfma_preshuffle", _tensor_cache_key(b_q))
    cached = _B_MFMA_PRESHUFFLE_CACHE.get(key)
    if cached is not None:
        return cached
    ...
    if len(_B_MFMA_PRESHUFFLE_CACHE) >= 4:
        _B_MFMA_PRESHUFFLE_CACHE.clear()
    _B_MFMA_PRESHUFFLE_CACHE[key] = b_preshuffled
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if a.shape[0] >= 32 and (a.shape[0] % 32) == 0 and (a.shape[1] % 64) == 0 and (b.shape[0] % 32) == 0:
        ...
        b_packed, b_scale = _get_b_contract_mfma_fp4_live(b_q, b_scale_sh)
