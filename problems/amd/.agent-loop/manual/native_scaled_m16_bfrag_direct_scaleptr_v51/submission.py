...
CONFIG = {
    "variant_name": "native_scaled_m16_bfrag_direct_scaleptr_v51",
    "family": "hip_explore",
...
void mxfp4_mm_hip_mfma_scale_exact_m16_scaleptr(torch::Tensor a_packed, torch::Tensor b_packed, torch::Tensor a_scale, torch::Tensor b_scale, torch::Tensor c);
...
__global__ void mxfp4_mm_kernel_mfma_scale_exact_m16_scaleptr(
    const unsigned char* __restrict__ a_packed,
    const unsigned char* __restrict__ b_packed,
    const uint8_t* __restrict__ a_scale,
    const uint8_t* __restrict__ b_scale,
    ...
    const unsigned char* a_ptr = a_valid
        ? (a_packed + a_row * a_bytes_per_row + group4 * 16)
        : nullptr;
    const uint8_t* a_scale_ptr = a_valid
        ? (a_scale + a_row * a_scale_stride)
        : nullptr;
    const unsigned char* b_ptr = b_packed + (tile_col + lane16) * b_bytes_per_row + group4 * 16;
    ...
    for (int tile_k = 0; tile_k < k; tile_k += MFMA_K) {
        ...
        if (a_valid) {
            a_ptr += MFMA_K / 2;
            a_scale_ptr += MFMA_K / 32;
        }
        b_ptr += MFMA_K / 2;
        b_scale_ptr += MFMA_K / 32;
    }
...
void mxfp4_mm_hip_mfma_scale_exact_m16_scaleptr(torch::Tensor a_packed, torch::Tensor b_packed, torch::Tensor a_scale, torch::Tensor b_scale, torch::Tensor c) {
    ...
    dim3 grid((n + 16 - 1) / 16, (m + 16 - 1) / 16);
    hipLaunchKernelGGL(
        mxfp4_mm_kernel_mfma_scale_exact_m16_scaleptr,
        grid,
        block,
        0,
        0,
...
def _module():
    ...
    if _MODULE is None:
        ...
        _MODULE = load_inline(
            ...
            functions=["mxfp4_mm_hip", "mxfp4_mm_hip_mfma_medium", "mxfp4_mm_hip_mfma_scale_exact_m16", "mxfp4_mm_hip_mfma_scale_exact_m16_scaleptr", "mxfp4_mm_hip_mfma_scale_exact_m32", "mxfp4_pack_a_fixed", "mxfp4_repack_b_packed", "mxfp4_pack_b_m32_direct", "mxfp4_unshuffle_b_scale"],
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if a.shape[0] == 16 and (a.shape[1] % 128) == 0 and (b.shape[0] % 16) == 0:
        ...
        if len(inflight) > 64:
            del inflight[:-64]
        _phase("pre_direct_kernel_launch", chunked=False)
        mod.mxfp4_mm_hip_mfma_scale_exact_m16_scaleptr(a_packed, b_packed, a_scale, b_scale, c)
        _phase("post_wrapper_return", chunked=False)
        return c
