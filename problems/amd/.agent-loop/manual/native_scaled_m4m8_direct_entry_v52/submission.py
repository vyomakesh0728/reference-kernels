...
CONFIG = {
    "variant_name": "native_scaled_m4m8_direct_entry_v52",
    "family": "hip_explore",
...
def _module():
    ...
    if _MODULE is None:
        ...
        _MODULE = load_inline(
            ...
            functions=["mxfp4_mm_hip", "mxfp4_mm_hip_mfma_medium", "mxfp4_mm_hip_mfma_scale_exact_m16", "mxfp4_mm_hip_mfma_scale_exact_m4m8_direct_entry", "mxfp4_mm_hip_mfma_scale_exact_m32", "mxfp4_pack_a_fixed", "mxfp4_repack_b_packed", "mxfp4_pack_b_m32_direct", "mxfp4_unshuffle_b_scale"],
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if a.shape[0] in (4, 8) and (a.shape[1] % 128) == 0 and (b.shape[0] % 16) == 0:
        ...
        if len(inflight) > 64:
            del inflight[:-64]
        _phase("pre_direct_kernel_launch", chunked=False)
        mod.mxfp4_mm_hip_mfma_scale_exact_m4m8_direct_entry(
            a_in,
            b_q_u8,
            b_scale_u8,
            a_packed,
