...
CONFIG = {
    "variant_name": "dispatch_pack_sparse256_v4",
    "family": "hip_explore",
...
def _module():
    global MODULE
    if MODULE is None:
        build_root = Path(tempfile.gettempdir()) / "moe_dispatch_pack_sparse256_v4_build"
        build_root.mkdir(parents=True, exist_ok=True)
        digest = hashlib.sha1((CPP_WRAPPER + HIP_SRC).encode("utf-8")).hexdigest()[:12]
        module_name = f"moe_dispatch_pack_sparse256_v4_{digest}"
        MODULE = load_inline(
            name=module_name,
