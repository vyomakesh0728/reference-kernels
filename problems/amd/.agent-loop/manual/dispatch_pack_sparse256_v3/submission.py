...
CONFIG = {
    "variant_name": "dispatch_pack_sparse256_v3",
    "family": "hip_explore",
...
def _module():
    global MODULE
    if MODULE is None:
        build_root = Path(tempfile.gettempdir()) / "moe_dispatch_pack_sparse256_v3_build"
        build_root.mkdir(parents=True, exist_ok=True)
        digest = hashlib.sha1((CPP_WRAPPER + HIP_SRC).encode("utf-8")).hexdigest()[:12]
        module_name = f"moe_dispatch_pack_sparse256_v3_{digest}"
        MODULE = load_inline(
            name=module_name,
...
def _dispatch_split_shared_path(
    ...
    routed_out = _green_two_stage_exact(
        hidden_states,
        gate_up_weight_shuffled[: int(config["n_routed_experts"])].contiguous(),
        down_weight_shuffled[: int(config["n_routed_experts"])].contiguous(),
        gate_up_weight_scale_shuffled[: int(config["n_routed_experts"])].contiguous(),
        down_weight_scale_shuffled[: int(config["n_routed_experts"])].contiguous(),
        routed_weights,
        routed_ids,
        config,
    )
