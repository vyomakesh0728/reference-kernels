...
CONFIG = {
    "variant_name": "dispatch_pack_sparse256_v5",
    "family": "hip_explore",
...
def _module():
    global MODULE
    if MODULE is None:
        build_root = Path(tempfile.gettempdir()) / "moe_dispatch_pack_sparse256_v5_build"
        build_root.mkdir(parents=True, exist_ok=True)
        digest = hashlib.sha1((CPP_WRAPPER + HIP_SRC).encode("utf-8")).hexdigest()[:12]
        module_name = f"moe_dispatch_pack_sparse256_v5_{digest}"
        MODULE = load_inline(
            name=module_name,
...
def _dispatch_split_shared_path(
    ...
    shared_out = _shared_exact_path(
        ...
        topk_ids[:, routed_topk:].contiguous() - int(config["n_routed_experts"]),
        config,
    )
