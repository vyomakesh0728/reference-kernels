...
_TRITON_QUANT = None
CPP_WRAPPER = r"""
void moe_mxfp4_pack_a_fixed(torch::Tensor a, torch::Tensor a_packed, torch::Tensor a_scale);
...
void moe_mxfp4_unshuffle_b_scale(torch::Tensor b_scale_sh, torch::Tensor b_scale);
"""
...
__device__ __forceinline__ unsigned char apply_fixed_adjustment(unsigned char nibble, float q) {
    if (nibble == 0x1u && q <= 0.25f) return 0x0u;
    if (nibble == 0x3u && q <= 1.25f) return 0x2u;
    if (nibble == 0x5u && q <= 2.5f) return 0x4u;
    if (nibble == 0x7u && q <= 5.0f) return 0x6u;
    if (nibble == 0x9u && q > -0.251953125f) return 0x8u;
    if (nibble == 0xBu && q > -1.2578125f) return 0xAu;
    if (nibble == 0xDu && q > -2.515625f) return 0xCu;
    if (nibble == 0xFu && q > -5.03125f) return 0xEu;
    return nibble;
}
...
__global__ void moe_mxfp4_pack_a_fixed_kernel(
...
__global__ void moe_mxfp4_unshuffle_b_scale_kernel(
...
void moe_mxfp4_unshuffle_b_scale(torch::Tensor b_scale_sh, torch::Tensor b_scale) {
    ...
    hipLaunchKernelGGL(
        moe_mxfp4_unshuffle_b_scale_kernel,
        grid,
        block,
        0,
        0,
    ...
    for (int tile_k = 0; tile_k < k; tile_k += MFMA_K) {
        ...
        if (b_row < n) {
            ...
            for (int i = 0; i < 16; ++i) {
                ...
                b_buf.b[i] = fp4_pack(
                    (b_nibble == 0) ? (byte0 & 0xFu) : (byte0 >> 4),
                    (b_nibble == 0) ? (byte1 & 0xFu) : (byte1 >> 4)
                );
            }
        }
...
def _module():
    global _MODULE
    if _MODULE is None:
        build_root = Path(tempfile.gettempdir()) / "moe_scaled_mfma_correct_v6_build"
        build_root.mkdir(parents=True, exist_ok=True)
        digest = hashlib.sha1((CPP_WRAPPER + HIP_SRC).encode("utf-8")).hexdigest()[:12]
        module_name = f"moe_scaled_mfma_correct_v6_{digest}"
        _MODULE = load_inline(
            name=module_name,
            cpp_sources=[CPP_WRAPPER],
            cuda_sources=[HIP_SRC],
            functions=["moe_mxfp4_pack_a_fixed", "moe_mxfp4_scaled_mfma_exact_m16", "moe_mxfp4_unshuffle_b_scale"],
...
def _quantize_activation_mxfp4(activation: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    packed = _packed_weight_matrix(packed, rows, cols)
    scale = _scale_matrix(scale, rows, cols)
...
def _packed_weight_matrix(weight_packed: torch.Tensor, rows: int, k: int) -> torch.Tensor:
    ...
    cols = k // 2
    if packed.ndim == 2 and packed.shape[0] >= rows and packed.shape[1] >= cols:
        packed = packed[:rows, :cols].contiguous()
    elif packed.numel() == rows * cols:
        packed = packed.reshape(rows, cols).contiguous()
    elif packed.numel() % cols == 0:
        packed_rows = packed.numel() // cols
        matrix = packed.reshape(packed_rows, cols)
        if packed_rows >= rows:
            packed = matrix[:rows].contiguous()
        else:
            raise RuntimeError(f"cannot normalize packed matrix with numel={packed.numel()} rows={rows} k={k}")
    else:
        raise RuntimeError(f"cannot normalize packed matrix with numel={packed.numel()} rows={rows} k={k}")

    # Match the proven exact-m16 MM contract: B is packed from logical [N, K]
    # into byte rows over K after a logical transpose to [K, N].
    logical = fp4_utils.mxfp4_to_f32(packed)[:rows, :k].to(torch.float32)
    transposed = fp4_utils.f32_to_mxfp4(logical.t().contiguous()).contiguous().view(torch.uint8)
    expected_rows = k
    expected_cols = rows // 2
    if transposed.ndim == 2 and transposed.shape[0] >= expected_rows and transposed.shape[1] >= expected_cols:
        return transposed[:expected_rows, :expected_cols].contiguous()
    if transposed.numel() == expected_rows * expected_cols:
        return transposed.reshape(expected_rows, expected_cols).contiguous()
    raise RuntimeError(
        f"cannot normalize transposed packed matrix with numel={transposed.numel()} rows={rows} k={k}"
    )
...
def _scale_matrix(weight_scale: torch.Tensor, rows: int, k: int) -> torch.Tensor:
    ...
    scale_cols = k // 32
    if scale.ndim == 2 and scale.shape[0] >= rows and scale.shape[1] >= scale_cols:
        return scale[:rows, :scale_cols].contiguous()
...
def _b_scale_rowmajor_from_shuffled(weight_scale_shuffled: torch.Tensor, rows: int, k: int) -> torch.Tensor:
    scale_cols = k // 32
    src = weight_scale_shuffled.contiguous().view(torch.uint8)
    if src.ndim == 1:
        src = src.reshape(1, -1)
    elif src.ndim > 2:
        src = src.reshape(src.shape[0], -1)
    out = torch.empty((rows, scale_cols), dtype=torch.uint8, device=src.device)
    _module().moe_mxfp4_unshuffle_b_scale(src, out)
    return out
...
def _scaled_mfma_nt(
    ...
    weight_scale_shuffled: torch.Tensor,
) -> torch.Tensor:
    ...
    m = activations_bf16.shape[0]
    padded_m = ((m + 15) // 16) * 16
    if padded_m != m:
        activations_bf16 = F.pad(activations_bf16, (0, 0, 0, padded_m - m)).contiguous()
    a_packed, a_scale = _quantize_activation_mxfp4(activations_bf16)
    b_packed = _packed_weight_matrix(weight_packed, n, k)
    b_scale = _b_scale_rowmajor_from_shuffled(weight_scale_shuffled, n, k)
...
def _moe_scaled_mfma_correct_path(
    ...
    for idx, expert in enumerate(unique_experts.tolist()):
        ...
        stage1_full = _scaled_mfma_nt(
            ...
            gate_up_weight_scale_shuffled[expert],
        )
        ...
        stage2_full = _scaled_mfma_nt(
            fused,
            down_weight[expert],
            down_weight_scale_shuffled[expert],
        )
...
def custom_kernel(data: input_t) -> output_t:
    ...
    del gate_up_weight_scale, down_weight_scale, gate_up_weight_shuffled, down_weight_shuffled
