...
def _module():
    global _MODULE
    if _MODULE is None:
        build_root = Path(tempfile.gettempdir()) / "moe_scaled_mfma_correct_v2_build"
        build_root.mkdir(parents=True, exist_ok=True)
        digest = hashlib.sha1((CPP_WRAPPER + HIP_SRC).encode("utf-8")).hexdigest()[:12]
        module_name = f"moe_scaled_mfma_correct_v2_{digest}"
        _MODULE = load_inline(
            name=module_name,
...
def _quantize_activation_mxfp4(activation: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    activation_bf16 = activation.to(torch.bfloat16).contiguous()
    rows = activation_bf16.shape[0]
    cols = activation_bf16.shape[1]
    packed = torch.empty((rows, cols // 2), dtype=torch.uint8, device=activation_bf16.device)
    scale = torch.empty((rows, cols // 32), dtype=torch.uint8, device=activation_bf16.device)
