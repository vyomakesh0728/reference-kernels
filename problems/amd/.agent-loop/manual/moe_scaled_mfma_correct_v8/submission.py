...
def _module():
    global _MODULE
    if _MODULE is None:
        build_root = Path(tempfile.gettempdir()) / "moe_scaled_mfma_correct_v8_build"
        build_root.mkdir(parents=True, exist_ok=True)
        digest = hashlib.sha1((CPP_WRAPPER + HIP_SRC).encode("utf-8")).hexdigest()[:12]
        module_name = f"moe_scaled_mfma_correct_v8_{digest}"
        _MODULE = load_inline(
            name=module_name,
            cpp_sources=[CPP_WRAPPER],
            cuda_sources=[HIP_SRC],
            functions=["moe_mxfp4_pack_a_fixed", "moe_mxfp4_scaled_mfma_exact_m16"],
...
def _quantize_activation_mxfp4(activation: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    activation_bf16 = activation.contiguous()
    rows = activation_bf16.shape[0]
    cols = activation_bf16.shape[1]
    packed = torch.empty((rows, cols // 2), device=activation_bf16.device, dtype=torch.uint8)
    scale = torch.empty((rows, cols // 32), device=activation_bf16.device, dtype=torch.uint8)
    _module().moe_mxfp4_pack_a_fixed(activation_bf16, packed, scale)
    return packed, scale
...
def _e8m0_unshuffle(scale_sh_u8: torch.Tensor, rows: int, cols: int) -> torch.Tensor:
    scale_src = scale_sh_u8.contiguous().view(torch.uint8)
    torch._assert(scale_src.ndim == 2, "shuffled scale source must be rank-2")
    ...
    copy_rows = min(scale_src.shape[0], padded_rows)
    copy_cols = min(scale_src.shape[1], padded_cols)
    scale_padded[:copy_rows, :copy_cols] = scale_src[:copy_rows, :copy_cols]
...
def _expert_scale_from_shuffled(shuffled_scale: torch.Tensor, expert: int, rows: int, k: int) -> torch.Tensor:
    scale_cols = k // 32
    row0 = expert * rows
    row1 = row0 + rows
    return _e8m0_unshuffle(shuffled_scale[row0:row1], rows, scale_cols)
...
def _moe_scaled_mfma_correct_path(
    ...
    for idx, expert in enumerate(unique_experts.tolist()):
        ...
        stage1_full = _scaled_mfma_nt(
            ...
            _expert_scale_from_shuffled(
                gate_up_weight_scale_shuffled,
                expert,
                gate_up_weight.shape[1],
                hidden_inputs.shape[1],
            ),
        )
