...
CONFIG = {
    "variant_name": "native_scaled_exact_shape_m256_k1536_constbody_v135",
    "family": "hip_explore",
...
# Candidate Card
# shape: exact public m256, n3072, k1536 path
# deleted_cost_center: generic loop/setup and generic shuffled-scale remap arithmetic inside the public exact-m256 raw-scale kernel body
# expected_upside_source: keep the proven direct-entry + raw-b_scale_sh law, but specialize the benchmark-shape exact m256 body so the highest-throughput shape pays less loop/setup/address cost on every CTA
# why_larger_than_noise: m256 is still >27 us, and its path keeps dynamic n/k/scale-cols plus generic shuffled-scale remap even though k/n are fixed in the benchmark; constant-body should shave setup overhead without altering math
# touched_symbols_or_regions: exact-m256 k1536 shuffled-scale helper specialization; exact-m256 k1536 kernel specialization; exact-m256 direct-entry dispatch only
# forbidden_edits: do not touch A-pack ownership, do not touch m4/m16/m32/m64 contracts, do not add helper launches or new scheduling regimes
# success_gate: improve m256 without regressing geomean
SCALE_GROUP = 32
...
__device__ __forceinline__ uint8_t mxfp4_load_unshuffled_b_scale_exact_m16_k7168(
    const uint8_t* __restrict__ b_scale_sh,
    int src_rows,
    int src_cols,
    int out_row,
    ...
    constexpr int COLS = 224;
    constexpr int PADDED_COLS = 224;
    constexpr int COL_BLOCKS = PADDED_COLS / 8;
    constexpr int ROWS = 2112;
    const int row_block = out_row >> 5;
    const int row_in_block = out_row & 31;
    const int d = row_in_block >> 4;
    const int b = row_in_block & 15;
...
__device__ __forceinline__ int pack_scale_e8m0x4_lane_from_shuffled_exact_m16_k7168(
    const uint8_t* __restrict__ b_scale_sh,
    int src_rows,
    int src_cols,
    int out_row,
    ...
    const uint8_t scale0 = mxfp4_load_unshuffled_b_scale_exact_m16_k7168(
        b_scale_sh, src_rows, src_cols, out_row, scale_block + group4
    );
    return static_cast<int>(scale0)
        | (127 << 8)
        | (127 << 16)
...
__global__ void mxfp4_mm_kernel_mfma_scale_exact_m16_k7168_n2112(
    ...
    for (int step = 0; step < (K / MFMA_K); ++step) {
        ...
        const int scale_block = step * 4;
        const int scale_a = pack_scale_e8m0x4_lane(a_scale_ptr, group4);
        const int scale_b = pack_scale_e8m0x4_lane_from_shuffled_exact_m16_k7168(
            b_scale_sh, src_rows, src_cols, tile_col + lane16, scale_block, group4
        );
        acc = __builtin_amdgcn_mfma_scale_f32_16x16x128_f8f6f4(a_buf.v, b_buf.v, acc, 4, 4, 0, scale_a, 0, scale_b);
...
__global__ void mxfp4_mm_kernel_mfma_scale_exact_m4_k512_dense(
    ...
    const unsigned char* b_row_ptr = b_packed + (tile_col + lane16) * 256 + group4 * 16;
    ...
    for (int step = 0; step < 4; ++step) {
        ...
        const int scale_a = a_active
            ...
            : (127 | (127 << 8) | (127 << 16) | (127 << 24));
        const int scale_b = pack_scale_e8m0x4_lane_from_shuffled_exact_m4_k512(
            b_scale_sh, src_cols, tile_col + lane16, scale_block, group4
        );
