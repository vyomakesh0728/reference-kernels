...
#!POPCORN gpu MI355X
# Candidate Card
# shape: exact public launch-law probe on the hot scored family (m4/m16/m32/m64/m256)
# regime_tag: host_extlaunch_r1
# deleted_cost_center: host-side kernel submission/marshalling overhead on the timed public exact A-pack + compute pair
# expected_upside_source: the live anchor still spends about 7-17 us of wall time outside the profiled GPU bodies, so keeping the exact kernels fixed and changing only the hot launch API is a direct probe of whether the remaining miss is in host submission law rather than MFMA math
# why_larger_than_noise: this branch preserves the exact ownership law, exact MFMA bodies, and B contracts while touching both timed launches on every scored exact shape; if the host launch path is part of the missing wall time, the effect should show portfolio-wide rather than as single-shape noise
# touched_symbols_or_regions: launch_mxfp4_pack_a_fixed_raw; exact public owned-workspace wrappers for m4/m16/m32/m64/m256; variant/load_inline surface
# forbidden_edits: do not change exact kernel bodies, A-pack semantics, workspace geometry, or B contracts
# success_gate: benchmark geomean improves by at least 0.5 us with visible movement on m64 and m256; otherwise kill the branch
...
CONFIG = {
    "variant_name": "host_extlaunch_r1",
    "family": "hip_explore",
...
inline void launch_kernel_via_ext(
    ...
    torch_check_hip_ok(
        hipExtLaunchKernel(function_address, grid, block, args, 0, 0, nullptr, nullptr, 0),
        phase
    );
}
...
inline void launch_mxfp4_pack_a_fixed_raw(
    ...
    if (USE_FP4_WAVE_PACK != 0) {
        ...
        int m_arg = m;
        int k_arg = k;
        int packed_stride_arg = packed_stride;
        int scale_stride_arg = scale_stride;
        void* args[] = {
            &a_ptr,
            &a_packed_ptr,
            &a_scale_ptr,
        ...
        launch_kernel_via_ext(
            reinterpret_cast<const void*>(mxfp4_pack_a_fixed_kernel_wave),
            grid,
            block,
            args,
            "launch mxfp4_pack_a_fixed_kernel_wave"
        );
        return;
    }
    ...
    launch_kernel_via_ext(
        reinterpret_cast<const void*>(mxfp4_pack_a_fixed_kernel_scalar),
        grid,
        block,
        args,
        "launch mxfp4_pack_a_fixed_kernel_scalar"
    );
}
...
void mxfp4_mm_hip_mfma_scale_exact_m16_direct_entry_public_k7168_n2112_owned_workspace(
    ...
    int b_scale_rows = static_cast<int>(b_scale_sh.size(0));
    int b_scale_cols = static_cast<int>(b_scale_sh.size(1));
    void* args[] = {
        &a_packed_ptr,
        &b_q_ptr,
        &a_scale_ptr,
    ...
    launch_kernel_via_ext(
        reinterpret_cast<const void*>(mxfp4_mm_kernel_mfma_scale_exact_m16_k7168_n2112),
        grid,
        block,
        args,
        "launch mxfp4_mm_kernel_mfma_scale_exact_m16_k7168_n2112"
    );
}
...
void mxfp4_mm_hip_mfma_scale_exact_m4_direct_entry_public_k512_owned_workspace(
    ...
    launch_kernel_via_ext(
        reinterpret_cast<const void*>(mxfp4_mm_kernel_mfma_scale_exact_m4_k512_dense),
        grid,
        block,
        args,
        "launch mxfp4_mm_kernel_mfma_scale_exact_m4_k512_dense"
    );
}
...
void mxfp4_mm_hip_mfma_scale_exact_m32_direct_entry_public_k512_owned_workspace(
    ...
    launch_kernel_via_ext(
        reinterpret_cast<const void*>(mxfp4_mm_kernel_mfma_scale_exact_m32_rawb_k512_unrolled),
        grid,
        block,
        args,
        "launch mxfp4_mm_kernel_mfma_scale_exact_m32_rawb_k512_unrolled"
    );
}
...
void mxfp4_mm_hip_mfma_scale_exact_m64_direct_entry_public_k2048_n7168_owned_workspace(
    ...
    launch_kernel_via_ext(
        reinterpret_cast<const void*>(mxfp4_mm_kernel_mfma_scale_exact_m64_k2048_n7168_rawscale),
        grid,
        block,
        args,
        "launch mxfp4_mm_kernel_mfma_scale_exact_m64_k2048_n7168_rawscale"
    );
}
...
void mxfp4_mm_hip_mfma_scale_exact_m256_direct_entry_public_k1536_n3072_owned_workspace(
    ...
    launch_kernel_via_ext(
        reinterpret_cast<const void*>(mxfp4_mm_kernel_mfma_scale_exact_m256_k1536_n3072),
        grid,
        block,
        args,
        "launch mxfp4_mm_kernel_mfma_scale_exact_m256_k1536_n3072"
    );
}
