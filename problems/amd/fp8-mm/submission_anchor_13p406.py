...
#!POPCORN gpu MI355X
# Candidate Card
# shape: shared helper on the hot exact-shape family (m4/m16/m32/m64/m256)
# regime_tag: helper_bf16_compact_t27
# deleted_cost_center: remaining helper conversion/control and short-K overlaunch inside mxfp4_pack_a_fixed_kernel_wave
# expected_upside_source: the benchmark frontier is now ~13.956 us on t26r while the freshest t25 profile still shows the shared A-pack helper at about 71-81% of self CUDA across every hot shape, so the highest-confidence next step is to keep the exact bodies fixed and cut the helper's remaining fixup, live-range, and launch waste
# why_larger_than_noise: this branch preserves the current ownership law, exact MFMA bodies, and B contracts, restores the helper-only t26 launch/fixup win into the live file, and adds a new residual helper bucket via BF16-native builtin packing with shorter value live ranges
# touched_symbols_or_regions: apply_fixed_adjustment; mxfp4_pack_a_fixed_kernel_wave; launch_mxfp4_pack_a_fixed_raw; helper FP4 pack builtins; variant/load_inline surface
# forbidden_edits: do not reopen duplicate A-pack ownership rewrites; do not touch exact MFMA bodies or B contracts
# success_gate: benchmark geomean improves by at least 0.2 us with no visible shape regressing by more than 0.1 us
...
CONFIG = {
    "variant_name": "helper_bf16_compact_t27",
    "family": "hip_explore",
