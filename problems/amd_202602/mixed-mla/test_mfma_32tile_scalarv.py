...
#!POPCORN gpu MI355X
"""32-token tile MFMA QK^T + scalar V. Isolate whether 32-tile QK^T works."""
import os
...
__global__ void stage1(
    ...
    __shared__ uint8_t kvv[VD * TILE];  // transposed V buffer
    ...
    for (int ts = ms; ts < me; ts += TILE) {
        ...
        // Transpose V: kvv[dim * 32 + tok] = kv_lds[tok * KVS + dim]
        ...
        for (int hi = 0; hi < 4; hi++) {
            ...
            for (int t = 0; t < tsz; t++) {
                ...
                tsum += es;
                for (int d = 0; d < 32; d++) {
                    // Read V from transposed buffer: kvv[dim * 32 + token]
                    float vv = fp82f(kvv[(vbase + d) * TILE + t]) * kvs;
                    va[hi][d] += es * vv;
                }
            }
...
def _get():
    if _mod[0] is None:
        _mod[0] = load_inline(name='mla_32t_sv_kvv', cpp_sources='',
