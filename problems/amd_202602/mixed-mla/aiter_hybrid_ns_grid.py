...
#!POPCORN gpu MI355X
"""Hybrid a16w8/a8w8 with grid-searched ns per shape.
a16w8 for bs<64 OR kvl<=1024. a8w8 for bs>=64 AND kvl>=8192.
Aggressive low ns to minimize kernel launch overhead."""
import torch
...
def _get_config(bs, kvl):
    ...
    # Grid: try fewer splits everywhere
    if kvl <= 1024:
        if bs <= 4:   return (4, False, 2, True)
        if bs <= 32:  return (4, False, 2, True)
        if bs <= 64:  return (4, False, 2, True)
        return (4, False, 2, True)   # bs=256, kv=1k: a16w8, ns=4
    else:  # kvl = 8192
        if bs <= 4:   return (4, False, 2, True)   # a16w8, ns=4
        if bs <= 32:  return (8, False, 2, True)    # a16w8, ns=8
        if bs <= 64:  return (8, True, 1, False)    # a8w8, ns=8 (was 16)
        return (16, True, 1, False)                 # a8w8, ns=16 (was 32)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if use_a8w8:
        qi, qs = _quantize_fp8(q); qv = qi.view(-1,NUM_HEADS,QK_HEAD_DIM)
    else:
