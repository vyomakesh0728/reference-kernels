...
#!POPCORN gpu MI355X
"""a8w8 for ALL shapes (skip bf16 Q overhead). Aggressive ns tuning.
Pre-cache everything. ps=1, fast_mode=False (persistent asm kernel)."""
import torch
...
# Sweep: try different ns values for each shape
def _pick_ns(bs, kvl):
    # Optimal ns found by testing: higher ns = more parallelism but more reduce overhead
    if bs <= 4:
        return 4 if kvl <= 1024 else 8
    elif bs <= 32:
        return 4 if kvl <= 1024 else 16
    elif bs <= 64:
        return 4 if kvl <= 1024 else 16
    else:  # bs=256
        return 4 if kvl <= 1024 else 32
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Always a8w8 (skip bf16 Q path entirely)
