...
#!POPCORN gpu MI355X
"""Best aiter-only config found. ~60us geomean (down from 68us).

Key findings:
- a8w8+dynquant is best for bs>=32, kv=8k (a16w8 is 2x slower)
- ns=8-16 is sweet spot (ns=4 or 64 both worse)
- ps=1 works best (ps=2 or 4 show no benefit)
- Small shapes (kv<=1024): a16w8, ns=4-8
- Medium shapes (bs<=32, kv=8k): a8w8, ns=8
- Large shapes (bs>=64, kv=8k): a8w8, ns=8-16

The wall: bs=256,kv=8k at ~312us.
...
def _get_config(bs, kvl):
    """Best config from exhaustive grid search."""
    if kvl <= 1024:
