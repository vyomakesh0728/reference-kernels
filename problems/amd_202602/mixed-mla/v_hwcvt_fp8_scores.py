...
#!POPCORN gpu MI355X
"""MFMA QK^T (32-tile) + V MFMA with hardware fp8 score conversion (v_cvt_pk_fp8_f32)."""
import os
...
// Hardware fp8 conversion: packs 2 f32 values into fp8 bytes within an int32
__device__ __forceinline__ int cvt_pk_fp8_lo(float a, float b, int old) {
    return __builtin_amdgcn_cvt_pk_fp8_f32(a, b, old, false);  // lower 2 bytes
}
__device__ __forceinline__ int cvt_pk_fp8_hi(float a, float b, int old) {
    return __builtin_amdgcn_cvt_pk_fp8_f32(a, b, old, true);   // upper 2 bytes
}
...
__global__ void stage1(
    ...
    __shared__ uint8_t sc8[NH * TILE];
    __shared__ uint8_t kvv[VD * TILE];
    ...
    for (int ts = ms; ts < me; ts += TILE) {
        ...
        // Online softmax + score quantization via hardware fp8 conversion
        v4f corr_v;
        for (int hi = 0; hi < 4; hi++) {
            int head = g * 4 + hi;
            float tmax = -1e20f;
            ...
            // Quantize scores to fp8 using hardware v_cvt_pk_fp8_f32
            // Pack 4 scores at a time into int32, then store as bytes
            for (int t = 0; t < TILE; t += 4) {
                ...
                int pk = cvt_pk_fp8_lo(s0, s1, 0);     // bytes 0,1
                pk = cvt_pk_fp8_hi(s2, s3, pk);          // bytes 2,3
                // Store 4 fp8 bytes
        ...
        // Transpose V: kvv[v_dim * 32 + token] = kv_lds[token * KVS + v_dim]
        ...
        // A = scores[head][token]: lane%16=head, k=tokens striped
        long sa = *(long*)(sc8 + l16 * TILE + g * 8);
        for (int vc = 0; vc < 32; vc++) {
            int vb = vc * 16;
            // B = V[token][v_dim]: lane%16=v_dim, k=tokens striped
...
def _get():
    if _mod[0] is None:
        _mod[0] = load_inline(name='mla_hwcvt_fp8_v2', cpp_sources='',
