...
#!POPCORN gpu MI355X
"""Full MFMA kernel: QK^T + V via V_MFMA_F32_16X16X32_FP8_FP8. 32-token tiles."""
import os
...
__global__ void stage1(
    ...
    for (int ts = ms; ts < me; ts += TILE) {
        ...
        // QK^T MFMA: tokens 0-15 and 16-31
        ...
        for (int hi = 0; hi < 4; hi++) {
            ...
            // Quantize to fp8 (max exp_score=1.0, scale=1/240)
            for (int t = 0; t < TILE; t++)
                sc8[head * TILE + t] = f2fp8(sc[head * TILE + t] * 240.0f);
        }
        ...
        // A = scores[head][token]: lane%16=head, k=tokens
        long sa = *(long*)(sc8 + l16 * TILE + g * 8);
        for (int vc = 0; vc < 32; vc++) {
            int vb = vc * 16;
            // B = V[token][v_dim]: lane%16=v_dim_in_chunk, k=tokens
    ...
    float vs = kvs / 240.0f;  // combined V scale
    for (int hi = 0; hi < 4; hi++) {
        int head = g * 4 + hi;
...
def _get():
    if _mod[0] is None:
        _mod[0] = load_inline(name='mla_mfma_full_o1', cpp_sources='',
