...
#!POPCORN gpu MI355X
"""V MFMA with bf8 (e5m2fnuz) score quantization for better dynamic range.

Problem: fp8 e4m3fnuz has min normal = 2^(-7) = 0.0078. With 240x multiplier,
scores below ~3.2e-5 round to zero, killing 95% of softmax weights.

Fix: Use bf8 e5m2fnuz for scores (A operand in V MFMA). bf8 has:
  - min normal = 2^(-15) ~ 3e-5, so with 240x multiplier: min score ~ 1.25e-7
  - 250x better dynamic range than fp8 e4m3fnuz
  - Only 2 mantissa bits (less precision), but attention is dominated by top-k anyway

MFMA layout:
  - QK^T: __builtin_amdgcn_mfma_f32_16x16x32_fp8_fp8 (both fp8 e4m3)
  - V:    __builtin_amdgcn_mfma_f32_16x16x32_bf8_fp8  (A=bf8 scores, B=fp8 V)
"""
import os
...
// fp8 e4m3fnuz: bias=8, 4 exponent bits, 3 mantissa bits
// Used for Q, K, V data
__device__ __forceinline__ uint8_t f2fp8(float x) {
    if (x == 0.0f) return 0;
    union { float f; uint32_t u; } c; c.f = x;
    uint32_t s = (c.u >> 31) & 1;
    int e = (int)((c.u >> 23) & 0xFF) - 119;  // IEEE bias 127 - fp8 bias 8 = 119
    uint32_t m = (c.u >> 20) & 0x7;            // 3 mantissa bits
    if (e <= 0) return 0;
...
// Used for softmax scores (A operand in V MFMA)
// Min positive normal: 2^(1-16) = 2^(-15) ~ 3.05e-5
// Max finite: (1 + 0.75) * 2^(30-16) = 1.75 * 2^14 = 28672
// With 240x multiplier: min representable score ~ 1.27e-7
__device__ __forceinline__ uint8_t f2bf8(float x) {
    if (x == 0.0f) return 0;
    union { float f; uint32_t u; } c; c.f = x;
    uint32_t s = (c.u >> 31) & 1;
    // IEEE exponent -> bf8 exponent: IEEE_bias(127) - bf8_bias(16) = 111
    int e = (int)((c.u >> 23) & 0xFF) - 111;
    uint32_t m = (c.u >> 21) & 0x3;  // 2 mantissa bits
    // Rounding: check bit 20 (first truncated bit)
    uint32_t round_bit = (c.u >> 20) & 1;
    m += round_bit;
    if (m > 3) { m = 0; e += 1; }  // mantissa overflow -> bump exponent
    if (e <= 0) return 0;           // underflow to zero
    if (e >= 32) return (s << 7) | 0x7F;  // saturate to max
    return (s << 7) | ((uint32_t)e << 2) | m;
}
...
// ---- Stage 1: split-K attention with MFMA ----
...
__global__ void stage1(
    ...
    __shared__ uint8_t sc8[NH * TILE];          // 16 * 32 = 512 (bf8 scores)
    ...
    for (int ts = ms; ts < me; ts += TILE) {
        ...
        // Load KV tile to LDS (QKD bytes per token, padded to KVS)
        ...
        // ---- QK^T MFMA: fp8 x fp8 ----
        // D[head][token]: lane%16=token(col), head=4*(lane/16)+gpr
        // tokens 0-15 and 16-31
        ...
        // Store scores to shared: sc[head][token]
        for (int hi = 0; hi < 4; hi++) {
            sc[(g*4+hi) * TILE + l16]        = qk1[hi] * cqk;
            sc[(g*4+hi) * TILE + HALF + l16] = qk2[hi] * cqk;
        }

        // ---- Online softmax + bf8 score quantization per head ----
        v4f corr_v;
        for (int hi = 0; hi < 4; hi++) {
            int head = g * 4 + hi;
            float tmax = -1e20f;
            ...
            // Quantize exp_scores to bf8 e5m2fnuz (240x multiplier)
            // bf8 min normal * 240 ~ 3e-5 * 240 ~ 0.0073
            // -> captures scores down to ~1.27e-7 (vs fp8's ~3.2e-5)
            for (int t = 0; t < TILE; t++)
                sc8[head * TILE + t] = f2bf8(sc[head * TILE + t] * 240.0f);
        }

        // Correct V accumulators (online softmax rescaling)
        for (int vc = 0; vc < 32; vc++) va[vc] *= corr_v;

        // ---- Transpose V data: kvv[v_dim][token] = kv_lds[token][v_dim] ----
        ...
        // ---- V MFMA: bf8 scores x fp8 V ----
        // A = scores[head][token]: bf8 e5m2fnuz
        //   lane%16 = head (row), k = tokens (32 per MFMA)
        // B = V[v_dim][token]: fp8 e4m3fnuz (transposed)
        //   lane%16 = v_dim_in_chunk (col), k = tokens (32 per MFMA)
        // D[head][v_dim]: lane%16 = v_dim (col), head = 4*(lane/16) + gpr
    ...
    // ---- Write output ----
    // D[head][v_dim]: lane%16=v_dim, head=4*g+gpr
    float vs = kvs / 240.0f;  // combined V scale: kv_scale / score_multiplier
    for (int hi = 0; hi < 4; hi++) {
        int head = g * 4 + hi;
...
// ---- Reduce: combine splits via log-sum-exp ----
...
def _get():
    if _mod[0] is None:
        _mod[0] = load_inline(name='mla_mfma_bf8_scores', cpp_sources='',
