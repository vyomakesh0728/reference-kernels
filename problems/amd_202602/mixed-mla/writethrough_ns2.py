...
#!POPCORN gpu MI355X
"""Write-through v2: ns=2 for small KV (compromise between reduce overhead and parallelism).

Previous finding: ns=1 kills parallelism, making kv=1024 shapes 2-3x slower.
Hypothesis: ns=2 balances reduce overhead vs parallelism.

Config strategy:
- kv <= 1024: ns=2 (minimal reduce overhead, still some parallelism)
...
def _get_config(bs, kvl):
    """ns=2 for small kv, ns=32 for large kv."""
    if kvl <= 1024:
        # Try ns=2: minimal reduce overhead but still some parallelism
        # Keep a16w8 for small shapes
