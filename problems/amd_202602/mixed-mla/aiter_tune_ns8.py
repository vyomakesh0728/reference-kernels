...
#!POPCORN gpu MI355X
"""Aiter tuning: try ns=8 for all large shapes instead of ns=16.

Hypothesis: ns=16 may have too much reduce overhead. ns=8 is "sweet spot" per grid search.
"""
...
def _get_config(bs, kvl):
    """Changed: ns=8 for all kv=8k instead of ns=16 for large batch."""
    if kvl <= 1024:
        if bs <= 32:
        ...
        # Changed: all large shapes use ns=8 instead of 16
