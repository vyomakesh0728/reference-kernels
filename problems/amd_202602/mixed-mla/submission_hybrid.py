...
#!POPCORN gpu MI355X
"""Aiter submission with per-shape config tuning.

Changes from baseline (68us geomean):
- kv=8192: ns=16 instead of 32 (less reduce overhead)
- bs=256, kv=1024: ns=8 instead of 4 (more split-K parallelism)
- bs=4, kv=8192: a16w8 (skip Q quant for only 4 queries)
All fp8 KV, lazy caching (no eager warmup to avoid compilation timeout).
"""
...
def _get_config(bs, kvl):
    """Per-shape tuning: (ns, use_a8w8, ps, fm)."""
    if kvl <= 1024:
        if bs <= 32:
