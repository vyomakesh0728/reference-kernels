...
#!POPCORN gpu MI355X
"""Phase 5: LDS KV caching — eliminate global memory latency from MFMA path.

The key insight we missed: each MFMA stalls ~400 cycles waiting for global
memory loads, but the MFMA itself takes only ~8 cycles. 98% of execution
time is memory stalls with zero latency hiding (1 wavefront).

Fix: Load 16 tokens × 576 bytes = 9.2KB into LDS ONCE, then run all 50
MFMAs from LDS (~10 cycle latency). This turns the kernel from
memory-latency-bound to compute/bandwidth-bound.

LDS layout:
  q_lds:  [16][580] bf16 = 18.6KB   (Q cached once)
  kv_lds: [16][576] uint8 = 9.2KB   (KV per 16-token block)
  scores: [16][16] float = 1KB      (softmax weights)
  Total: ~29KB (fits in 160KB)
"""
...
constexpr int KV_PAD = 576;  // no padding needed, 576 is fine
...
// Branchless fp8 E4M3 FNUZ → f32 via bit manipulation
...
//   q_lds:      uint16[16][Q_PAD]   = 18560 bytes
//   kv_lds:     uint8[16][KV_PAD]   = 9216 bytes
//   scores_lds: float[16][16]       = 1024 bytes
//   Total: 28800 bytes ≈ 28.1 KB
...
__global__ void mla_fused_kernel(
    ...
    auto* scores_lds = reinterpret_cast<float*>(smem + N_HEADS * Q_PAD * 2 + 16 * KV_PAD);

    // Load Q into LDS (one-time)
    ...
    if (s_start < kv_len) {
        for (int t_base = s_start; t_base < s_end; t_base += 16) {
            ...
            // ===== LOAD KV INTO LDS =====
            // 16 tokens × 576 bytes = 9216 bytes, 64 threads → 144 bytes each
            for (int i = lane; i < 16 * KV_PAD; i += WARP) {
                int tok = i / KV_PAD;
                int dim = i % KV_PAD;
                int abs_tok = kv_start + t_base + tok;
                kv_lds[tok * KV_PAD + dim] = (tok < t_count) ? kv[abs_tok * QK_DIM + dim] : 0;
            }
            __syncthreads();
            ...
            for (int tk = 0; tk < N_QK_TILES; tk++) {
                ...
                // B: K from KV_LDS (fp8→bf16)
                _B16x8 b_reg;
                if (valid) {
                    const uint8_t* kp = &kv_lds[lane16 * KV_PAD + k_base];
                    b_reg.xy[0] = {
            ...
            for (int vt = 0; vt < N_V_TILES; vt++) {
                ...
                // A: scores from LDS (transposed)
                _B16x4 a_v;
                #pragma unroll
                for (int b = 0; b < 4; b++)
                    a_v[b] = f32_to_bf16_bits(scores_lds[lane16 * 16 + 4 * group + b]);

                // B: V from KV_LDS (fp8→bf16 with scale)
                _B16x4 b_v;
                #pragma unroll
                for (int b = 0; b < 4; b++) {
                    ...
                    if (tok_off < t_count && vd < V_DIM) {
                        b_v[b] = f32_to_bf16_bits(fp8_fast(kv_lds[tok_off * KV_PAD + vd]) * kv_scale);
                    } else {
                        b_v[b] = 0;
                    }
                }
...
void mla_fused(
    ...
    int smem_bytes = N_HEADS * Q_PAD * 2 + 16 * KV_PAD + N_HEADS * 16 * sizeof(float);
    dim3 block(64);
...
def _module():
    global _MODULE
    if _MODULE is None:
        build_root = Path(tempfile.gettempdir()) / "mla_p5_build"
        build_root.mkdir(parents=True, exist_ok=True)
        digest = hashlib.sha1((CPP_WRAPPER + HIP_SRC).encode()).hexdigest()[:12]
        _MODULE = load_inline(
            name=f"mla_p5_{digest}",
