...
#!POPCORN gpu MI355X
"""
Minimal test: Does tl.dot_scaled compile on gfx950?

This kernel ONLY tests dot_scaled compilation.
The actual MLA computation falls back to aiter.
"""
...
def test_dot_scaled():
    """Test if tl.dot_scaled compiles."""
    ...
    try:
        ...
        def _test_kernel(
            A,  # (M, K) bf16
            B,  # (K, N) uint8 packed fp4
            B_scale,  # (K//32, N) uint8 e8m0
            C,  # (M, N) float32
            M: tl.constexpr,
            N: tl.constexpr,
            K: tl.constexpr,
            ...
            # B is packed fp4: (K, N//2) with 2 values per byte
            # For testing, just load as-is
            b = tl.load(
                B + offs_k[:, None] * (N // 2) + tl.arange(0, N // 2)[None, :]
            )  # (K, N//2)

            # Scale: (K//32, N)
            b_scale = tl.load(B_scale + (offs_k // 32)[:, None] * N + offs_n[None, :])

            # Try dot_scaled
            # lhs = a (bf16), rhs = b (e2m1/fp4), rhs_scale = b_scale (e8m0)
            acc = tl.zeros((M, N), dtype=tl.float32)

            # NOTE: dot_scaled signature may vary - this is experimental
            # Try the call to see if it compiles
        ...
        # Try to compile with small shapes
        M, N, K = 16, 32, 64
        A = torch.randn((M, K), dtype=torch.bfloat16, device="cuda")
        B = torch.randint(0, 256, (K, N // 2), dtype=torch.uint8, device="cuda")
        B_scale = torch.full((K // 32, N), 127, dtype=torch.uint8, device="cuda")
        C = torch.zeros((M, N), dtype=torch.float32, device="cuda")

        _test_kernel[(1,)](A, B, B_scale, C, M, N, K)
        torch.cuda.synchronize()

        print("DOT_SCALED: COMPILATION SUCCEEDED!", file=sys.stderr)
