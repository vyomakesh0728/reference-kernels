...
#!POPCORN gpu MI355X
"""Targeted config variant:
- Keep current best path for kv=1024
- Force kv=8192 to a8w8 + ns=32 for all batch sizes
"""
...
def _get_config(bs: int, kvl: int):
    if kvl <= 1024:
        if bs <= 32:
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if use_a8w8:
        bkey = ("dq", q.numel(), q.device.index if q.device.index is not None else -1)
