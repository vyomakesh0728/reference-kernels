...
#!POPCORN gpu MI355X
"""Phase 4: Hardware fp8 dequant + vectorized loads + aggressive split-K.

Changes from Phase 3:
- __builtin_amdgcn_cvt_f32_fp8 for hardware fp8→f32 (1 instr vs ~10)
- Vectorized uint32 loads (4 fp8 bytes per load)
- Aggressive split-K: target 64+ blocks for GPU saturation
- Tuned split counts per shape
"""
...
// Hardware fp8 E4M3 FNUZ → f32 (byte_sel must be compile-time constant)
template<int SEL>
__device__ __forceinline__ float hw_fp8(uint32_t packed) {
    return __builtin_amdgcn_cvt_f32_fp8(packed, SEL);
}

// Convenience: convert 4 packed fp8 to 4 floats
__device__ __forceinline__ void hw_fp8x4(uint32_t pk, float& f0, float& f1, float& f2, float& f3) {
    f0 = hw_fp8<0>(pk);
    f1 = hw_fp8<1>(pk);
    f2 = hw_fp8<2>(pk);
    f3 = hw_fp8<3>(pk);
...
__global__ void mla_fused_kernel(
    ...
    if (s_start < kv_len) {
        for (int t_base = s_start; t_base < s_end; t_base += 16) {
            ...
            const uint8_t* my_kv = valid ? &kv[abs_tok * QK_DIM] : nullptr;
            ...
            for (int tk = 0; tk < N_QK_TILES; tk++) {
                ...
                // B: K from global, vectorized fp8→bf16
                _B16x8 b_reg;
                if (valid) {
                    uint32_t pk0 = *reinterpret_cast<const uint32_t*>(my_kv + k_base);
                    uint32_t pk1 = *reinterpret_cast<const uint32_t*>(my_kv + k_base + 4);
                    b_reg.xy[0] = {
            ...
            for (int vt = 0; vt < N_V_TILES; vt++) {
                ...
                // B: V from global, fp8→bf16 with branchless dequant
                _B16x4 b_v;
                #pragma unroll
                for (int b = 0; b < 4; b++) {
                    ...
                    if (tok_off < t_count) {
                        ...
                        uint32_t f32_bits = (static_cast<uint32_t>(raw & 0x80) << 24)
                            | ((static_cast<uint32_t>((raw >> 3) & 0xF) + 119u) << 23)
                            | (static_cast<uint32_t>(raw & 0x7) << 20);
                        float f = (raw == 0) ? 0.0f : __builtin_bit_cast(float, f32_bits);
...
def _module():
    global _MODULE
    if _MODULE is None:
        build_root = Path(tempfile.gettempdir()) / "mla_p4_build"
        build_root.mkdir(parents=True, exist_ok=True)
        digest = hashlib.sha1((CPP_WRAPPER + HIP_SRC).encode()).hexdigest()[:12]
        _MODULE = load_inline(
            name=f"mla_p4_{digest}",
