...
#!POPCORN gpu MI355X
"""Phase 10: Template-specialized kernels per shape group.

Optimizations over P9:
1. Template NUM_SPLITS as compile-time constant (compiler unrolls reduce)
2. Remove __syncthreads after V MFMA (1 wavefront = no sync needed)
3. Remove __syncthreads after scores write (same wavefront reads it)
4. Keep only the mandatory sync after KV LDS load (global→LDS visibility)
5. Eliminate runtime branching in V path (vd < V_DIM always true for 512/16=32 tiles)
6. Eliminate tok_off < t_count branch for full 16-token blocks
"""
...
CPP_WRAPPER = """
void mla_ns4(torch::Tensor q, torch::Tensor kv, torch::Tensor kv_scale, torch::Tensor out, torch::Tensor kv_indptr, torch::Tensor sb, torch::Tensor sl, torch::Tensor sc, int bs, float sm);
void mla_ns8(torch::Tensor q, torch::Tensor kv, torch::Tensor kv_scale, torch::Tensor out, torch::Tensor kv_indptr, torch::Tensor sb, torch::Tensor sl, torch::Tensor sc, int bs, float sm);
...
constexpr int Q_PAD = 576;  // no padding — saves LDS
...
__global__ void mla_kernel(
    ...
    for (int t_base = s_start; t_base < s_end; t_base += 16) {
        ...
        if (is_full_block) {
            // Full 16 tokens — no bounds check needed
            #pragma unroll
            for (int i = 0; i < 9; i++) {
        ...
        for (int tk = 0; tk < 18; tk++) {
            ...
            _B16x8 b_reg;
            if (valid) {
                uint32_t pk0 = *reinterpret_cast<const uint32_t*>(&kv_lds[lane16 * KV_STRIDE + k_base]);
                uint32_t pk1 = *reinterpret_cast<const uint32_t*>(&kv_lds[lane16 * KV_STRIDE + k_base + 4]);
        ...
        // No __syncthreads needed — single wavefront, scores_lds is coherent
        ...
        for (int vt = 0; vt < 32; vt++) {
            ...
            for (int b = 0; b < 4; b++) {
                ...
                // V_DIM=512 = 32*16, so vd < V_DIM always true
...
// Instantiate for each split count
...
void mla_ns##NS(torch::Tensor q, torch::Tensor kv, torch::Tensor kv_scale, \
    ...
    int smem = N_HEADS * Q_PAD * 2 + KV_BLOCK_BYTES + N_HEADS * 16 * sizeof(float); \
...
def _module():
    global _MODULE
    if _MODULE is None:
        build_root = Path(tempfile.gettempdir()) / "mla_p10_build"
        build_root.mkdir(parents=True, exist_ok=True)
        digest = hashlib.sha1((CPP_WRAPPER + HIP_SRC).encode()).hexdigest()[:12]
        _MODULE = load_inline(
            name=f"mla_p10_{digest}",
