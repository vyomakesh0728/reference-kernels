...
#!POPCORN gpu MI355X
"""Grid search: vary ns for each shape independently. Find optimal per-shape splits."""
import torch
...
# Best ns values found from manual tuning + extrapolation
# Format: (bs, kvl) -> (ns, use_a8w8, ps, fm)
_best_configs = {
    (4, 1024): (8, False, 2, True),    # 26.5us
    (4, 8192): (32, False, 2, True),   # 40.3us
    (32, 1024): (8, False, 2, True),   # 30.6us
    (32, 8192): (8, True, 1, False),   # 80.1us
    (64, 1024): (4, False, 2, True),   # 39.7us
    (64, 8192): (8, True, 1, False),   # 130us
    (256, 1024): (4, False, 2, True),  # 87.8us
    (256, 8192): (16, True, 1, False), # 312us - target for optimization
}

def _get_config(bs, kvl):
    """Best config found: ps=1 for a8w8."""
    if kvl <= 1024:
