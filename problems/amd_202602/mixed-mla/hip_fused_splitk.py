#!POPCORN leaderboard amd-mixed-mla
#!POPCORN gpu MI355X
"""Fused split-K HIP kernel: stage1 + reduce in ONE kernel launch.

Key optimizations:
1. Single kernel launch (no Python overhead between stage1 and reduce)
2. Atomic completion counter - last block per batch does the reduce
3. MFMA for both QK (fp8) and V (bf16)
4. FP8 E4M3 format (bias=7)
5. Pre-allocated all buffers
"""

from __future__ import annotations
import os
import sys

os.environ.setdefault("PYTORCH_ROCM_ARCH", "gfx950")

import torch
from torch.utils.cpp_extension import load_inline
from task import input_t, output_t

NUM_HEADS = 16
NUM_KV_HEADS = 1
QK_HEAD_DIM = 576
V_HEAD_DIM = 512
SM_SCALE = 1.0 / (QK_HEAD_DIM**0.5)

_HIP_SRC = r"""
#include <torch/extension.h>
#include <hip/hip_runtime.h>
#include <cstdint>

typedef float v4f __attribute__((ext_vector_type(4)));
typedef short v8s __attribute__((ext_vector_type(8)));

#define NH 16
#define QKD 576
#define VD 512
#define TILE 32
#define HALF 16
#define KVS 580

__device__ __forceinline__ float bf2f(uint16_t b) {
    union { uint32_t u; float f; } c;
    c.u = ((uint32_t)b) << 16;
    return c.f;
}

__device__ __forceinline__ uint16_t f2bf(float v) {
    union { float f; uint32_t u; } c;
    c.f = v;
    c.u += 0x7FFF + ((c.u >> 16) & 1);
    return (uint16_t)(c.u >> 16);
}

__device__ __forceinline__ uint16_t f2bf_trunc(float v) {
    union { float f; uint32_t u; } c;
    c.f = v;
    return (uint16_t)(c.u >> 16);
}

// FP8 E4M3 with bias=7 (OCP standard)
__device__ __forceinline__ uint8_t f2fp8_e4m3(float x) {
    x = fminf(fmaxf(x, -448.0f), 448.0f);
    if (fabsf(x) < 1.52587890625e-5f) return 0;
    union { float f; uint32_t u; } c; c.f = x;
    uint32_t s = (c.u >> 31) & 1;
    int e = (int)((c.u >> 23) & 0xFF) - 127 + 7;
    uint32_t m = (c.u >> 20) & 0x7;
    if (e <= 0) return (s << 7);
    if (e >= 15) return (s << 7) | 0x7E;
    return (s << 7) | (e << 3) | m;
}

__device__ __forceinline__ float fp8_e4m3_2f(uint8_t x) {
    if ((x & 0x7F) == 0) return 0.0f;
    uint32_t s = ((uint32_t)(x & 0x80)) << 24;
    uint32_t e = (x >> 3) & 0xF;
    uint32_t m = x & 0x7;
    if (e == 0) return 0.0f;
    uint32_t ieee = s | ((e + 120) << 23) | (m << 20);
    union { uint32_t u; float f; } c; c.u = ieee;
    return c.f;
}

__device__ __forceinline__ float warp_max64(float v) {
    for (int m = 32; m > 0; m >>= 1)
        v = fmaxf(v, __shfl_xor(v, m));
    return v;
}

// Fused split-K kernel: stage1 + atomic counter + last-block reduce
// Grid: (ns, bs), Block: (64)
__global__ void mla_fused_splitk(
    const uint16_t* __restrict__ Q,
    const uint8_t* __restrict__ KV,
    float kvs,
    const int32_t* __restrict__ kvi,
    const int32_t* __restrict__ qo,
    float* __restrict__ sO,           // [bs, ns, NH, VD] partial outputs
    float* __restrict__ sL,           // [bs, ns, NH] partial LSEs
    uint16_t* __restrict__ Out,       // [bs, NH, VD] final output
    int* __restrict__ done_count,     // [bs] atomic completion counters
    int ns, float sms
) {
    __shared__ uint8_t kv_lds[TILE * KVS];
    __shared__ uint8_t q8[NH * QKD];
    __shared__ float sc[NH * TILE];
    __shared__ uint16_t sc_bf16[NH * TILE];
    __shared__ uint16_t kvv_bf16[VD * TILE];

    const int tid = threadIdx.x;
    const int sid = blockIdx.x;  // split id
    const int bid = blockIdx.y;  // batch id
    const int g = tid >> 4, l16 = tid & 15;

    const int k0 = kvi[bid], k1 = kvi[bid + 1];
    const int slen = (k1 - k0 + ns - 1) / ns;
    const int ms = k0 + sid * slen;
    const int me = min(ms + slen, k1);

    // Handle empty splits
    if (ms >= k1) {
        for (int hi = 0; hi < 4; hi++) {
            int head = g * 4 + hi;
            if (l16 == 0) sL[(bid * ns + sid) * NH + head] = -1e20f;
            int ob = ((bid * ns + sid) * NH + head) * VD;
            for (int d = l16; d < VD; d += 16) sO[ob + d] = 0.0f;
        }
        goto do_reduce;
    }

    {
        // Load and quantize Q
        const int q0 = qo[bid];
        const uint16_t* qp = Q + static_cast<int64_t>(q0) * NH * QKD;
        
        float amax = 0.0f;
        for (int i = tid; i < NH * QKD; i += 64)
            amax = fmaxf(amax, fabsf(bf2f(qp[i])));
        amax = warp_max64(amax);
        
        float qs = fmaxf(amax, 1e-12f) / 448.0f;
        float inv_qs = 448.0f / fmaxf(amax, 1e-12f);
        
        for (int i = tid; i < NH * QKD; i += 64)
            q8[i] = f2fp8_e4m3(bf2f(qp[i]) * inv_qs);
        __syncthreads();

        // V accumulators
        v4f va[32];
        for (int i = 0; i < 32; i++) va[i] = (v4f){0, 0, 0, 0};
        float rmax[4] = {-1e20f, -1e20f, -1e20f, -1e20f};
        float rsum[4] = {0, 0, 0, 0};
        float cqk = qs * kvs * sms;

        // Process KV tiles
        for (int ts = ms; ts < me; ts += TILE) {
            int tsz = min(TILE, me - ts);

            // Load KV tile
            for (int i = tid; i < tsz * QKD; i += 64)
                kv_lds[(i / QKD) * KVS + (i % QKD)] = KV[(ts + i / QKD) * QKD + (i % QKD)];
            if (tsz < TILE)
                for (int i = tid; i < (TILE - tsz) * QKD; i += 64)
                    kv_lds[(tsz + i / QKD) * KVS + (i % QKD)] = 0;
            __syncthreads();

            // QK via MFMA
            v4f qk1 = (v4f){0,0,0,0}, qk2 = (v4f){0,0,0,0};
            for (int ch = 0; ch < 18; ch++) {
                int kb = ch * 32;
                long a = *(long*)(q8 + l16 * QKD + kb + g * 8);
                long b1 = *(long*)(kv_lds + l16 * KVS + kb + g * 8);
                long b2 = *(long*)(kv_lds + (HALF + l16) * KVS + kb + g * 8);
                qk1 = __builtin_amdgcn_mfma_f32_16x16x32_fp8_fp8(a, b1, qk1, 0, 0, 0);
                qk2 = __builtin_amdgcn_mfma_f32_16x16x32_fp8_fp8(a, b2, qk2, 0, 0, 0);
            }

            // Store scores
            for (int hi = 0; hi < 4; hi++) {
                sc[(g*4+hi) * TILE + l16] = qk1[hi] * cqk;
                sc[(g*4+hi) * TILE + HALF + l16] = qk2[hi] * cqk;
            }
            __syncthreads();

            // Online softmax + correction
            v4f corr_v;
            for (int hi = 0; hi < 4; hi++) {
                int head = g * 4 + hi;
                float tmax = -1e20f;
                for (int t = 0; t < tsz; t++)
                    tmax = fmaxf(tmax, sc[head * TILE + t]);
                float nm = fmaxf(rmax[hi], tmax);
                float c = __expf(rmax[hi] - nm);
                corr_v[hi] = c;
                rmax[hi] = nm;

                float tsum = 0.0f;
                for (int t = 0; t < TILE; t++) {
                    float e = (t < tsz) ? __expf(sc[head * TILE + t] - rmax[hi]) : 0.0f;
                    sc[head * TILE + t] = e;
                    tsum += e;
                }
                rsum[hi] = rsum[hi] * c + tsum;

                for (int t = 0; t < TILE; t++)
                    sc_bf16[head * TILE + t] = f2bf_trunc(sc[head * TILE + t]);
            }

            // Rescale V accumulators
            for (int vc = 0; vc < 32; vc++) va[vc] *= corr_v;

            // Load V values to bf16
            for (int i = tid; i < TILE * VD; i += 64) {
                int tok = i / VD;
                int dim = i % VD;
                float fv = fp8_e4m3_2f(kv_lds[tok * KVS + dim]) * kvs;
                kvv_bf16[dim * TILE + tok] = f2bf_trunc(fv);
            }
            __syncthreads();

            // V MFMA
            v8s sa;
            for (int k = 0; k < 8; k++)
                sa[k] = (short)sc_bf16[l16 * TILE + g * 8 + k];

            for (int vc = 0; vc < 32; vc++) {
                int vb = vc * 16;
                v8s sb;
                for (int k = 0; k < 8; k++)
                    sb[k] = (short)kvv_bf16[(vb + l16) * TILE + g * 8 + k];
                va[vc] = __builtin_amdgcn_mfma_f32_16x16x32_bf16(sa, sb, va[vc], 0, 0, 0);
            }
            __syncthreads();
        }

        // Write partial results
        for (int hi = 0; hi < 4; hi++) {
            int head = g * 4 + hi;
            int li = (bid * ns + sid) * NH + head;
            int ob = ((bid * ns + sid) * NH + head) * VD;
            if (l16 == 0)
                sL[li] = (rsum[hi] > 0.0f) ? (__logf(rsum[hi]) + rmax[hi]) : -1e20f;
            float inv = (rsum[hi] > 0.0f) ? (1.0f / rsum[hi]) : 0.0f;
            for (int vc = 0; vc < 32; vc++) {
                int vd = vc * 16 + l16;
                if (vd < VD)
                    sO[ob + vd] = va[vc][hi] * inv;
            }
        }
    }

do_reduce:
    __threadfence();
    
    // Atomic increment completion counter
    __shared__ int is_last;
    if (tid == 0) {
        int prev = atomicAdd(&done_count[bid], 1);
        is_last = (prev == ns - 1) ? 1 : 0;
    }
    __syncthreads();

    if (!is_last) return;

    // Last block does the reduce
    // Reset counter for next call
    if (tid == 0) done_count[bid] = 0;

    // Reduce across splits (64 threads, same layout as stage1)
    for (int hi = 0; hi < 4; hi++) {
        int head = g * 4 + hi;
        
        // Find global max LSE
        float ml = -1e20f;
        for (int s = 0; s < ns; s++)
            ml = fmaxf(ml, sL[(bid * ns + s) * NH + head]);
        
        // Accumulate weighted partial outputs
        float acc[32];
        for (int i = 0; i < 32; i++) acc[i] = 0.0f;
        float tw = 0.0f;
        
        for (int s = 0; s < ns; s++) {
            float l = sL[(bid * ns + s) * NH + head];
            if (l <= -1e19f) continue;
            float w = __expf(l - ml);
            tw += w;
            int base = ((bid * ns + s) * NH + head) * VD;
            for (int i = 0; i < 32; i++) {
                int vd = l16 * 32 + i;
                if (vd < VD)
                    acc[i] += w * sO[base + vd];
            }
        }
        
        // Write final output
        float inv = (tw > 0.0f) ? (1.0f / tw) : 0.0f;
        const int q0 = qo[bid];
        int ob = (q0 * NH + head) * VD;
        for (int i = 0; i < 32; i++) {
            int vd = l16 * 32 + i;
            if (vd < VD)
                Out[ob + vd] = f2bf(acc[i] * inv);
        }
    }
}

torch::Tensor mla_fwd(
    torch::Tensor Q,
    torch::Tensor KV,
    double kvs,
    torch::Tensor kvi,
    torch::Tensor qo,
    torch::Tensor so,
    torch::Tensor sl,
    torch::Tensor out,
    torch::Tensor done_count,
    int64_t bs,
    int64_t ns,
    double sms
) {
    mla_fused_splitk<<<dim3(ns, bs), 64>>>(
        (const uint16_t*)Q.data_ptr(),
        (const uint8_t*)KV.data_ptr(),
        (float)kvs,
        kvi.data_ptr<int32_t>(),
        qo.data_ptr<int32_t>(),
        so.data_ptr<float>(),
        sl.data_ptr<float>(),
        (uint16_t*)out.data_ptr(),
        done_count.data_ptr<int>(),
        (int)ns,
        (float)sms
    );
    return out;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("mla_fwd", &mla_fwd, "Fused split-K MLA decode");
}
"""

_hip_mod = None
_hip_failed = None
_buf_cache = {}


def _get_hip():
    global _hip_mod, _hip_failed
    if _hip_failed:
        return None
    if _hip_mod is not None:
        return _hip_mod
    try:
        _hip_mod = load_inline(
            name="mla_fused_splitk",
            cpp_sources="",
            cuda_sources=_HIP_SRC,
            extra_cuda_cflags=["-O3", "-ffast-math"],
            verbose=False,
        )
        _hip_failed = False
        return _hip_mod
    except Exception as e:
        _hip_failed = True
        print(f"[mla_fused_splitk] HIP JIT failed: {e}", file=sys.stderr)
        return None


def _get_ns(bs: int, kvl: int) -> int:
    if kvl <= 1024:
        return 8 if bs <= 32 else 4
    return 16 if bs <= 64 else 32


def custom_kernel(data: input_t) -> output_t:
    q, kv_data, qo_indptr, kv_indptr, config = data
    bs = int(config["batch_size"])
    kvl = int(config["kv_seq_len"])
    ns = _get_ns(bs, kvl)

    mod = _get_hip()
    if mod is None:
        raise RuntimeError("HIP compilation failed")

    kv_fp8, kv_scale = kv_data["fp8"]
    dev = q.device

    # Pre-allocate buffers (cached by shape)
    key = (bs, kvl, ns, dev)
    if key not in _buf_cache:
        _buf_cache[key] = {
            "out": torch.empty(
                (bs, NUM_HEADS, V_HEAD_DIM), dtype=torch.bfloat16, device=dev
            ),
            "so": torch.empty(
                (bs, ns, NUM_HEADS, V_HEAD_DIM), dtype=torch.float32, device=dev
            ),
            "sl": torch.empty((bs, ns, NUM_HEADS), dtype=torch.float32, device=dev),
            "done": torch.zeros(bs, dtype=torch.int32, device=dev),
        }
    buf = _buf_cache[key]

    kv_flat = kv_fp8.view(-1, QK_HEAD_DIM)

    mod.mla_fwd(
        q.view(-1, NUM_HEADS * QK_HEAD_DIM),
        kv_flat,
        float(kv_scale.item()),
        kv_indptr.to(torch.int32),
        qo_indptr.to(torch.int32),
        buf["so"],
        buf["sl"],
        buf["out"],
        buf["done"],
        bs,
        ns,
        SM_SCALE,
    )

    return buf["out"]
