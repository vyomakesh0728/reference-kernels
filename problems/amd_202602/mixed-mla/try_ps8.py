...
#!POPCORN gpu MI355X
"""Try page_size=8 like #1 might be doing."""
import torch
...
def _get_config(bs, kvl):
    """Try ps=8 for all shapes."""
    if kvl <= 1024:
        # a16w8 for small KV
        if bs <= 32: return (8, False, 8, True)
        if bs <= 64: return (4, False, 8, True)
        return (4, False, 8, True)
    else:  # kv=8192
        # a8w8 + ps=8 for large KV
        if bs <= 4:  return (32, False, 8, True)
        if bs <= 32: return (8, True, 8, False)
        if bs <= 64: return (8, True, 8, False)
        return (16, True, 8, False)
