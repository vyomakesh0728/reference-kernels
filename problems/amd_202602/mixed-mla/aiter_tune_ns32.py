...
#!POPCORN gpu MI355X
"""Aiter tuning: try ns=32 for largest shape (bs=256, kv=8k).

Hypothesis: More splits = more parallelism for memory-bound large shapes.
"""
...
def _get_config(bs, kvl):
    """Changed: ns=32 for bs>=128 instead of ns=16."""
    if kvl <= 1024:
        if bs <= 32:
        ...
        return (32, True, 1, False)  # Changed: ns=32 for bs=256
