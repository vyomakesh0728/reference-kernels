...
#!POPCORN gpu MI355X
"""Best hybrid: original small-case config + lower ns for large a8w8 cases."""
import torch
...
def _get_config(bs, kvl):
    # a16w8 for small: ps=2, fm=True, generous ns
    # a8w8 for large: ps=1, fm=False, lower ns than before
    if bs <= 32:
        ns = 8 if kvl <= 1024 else 8
        return (ns, False, 2, True)
    elif bs <= 64:
        if kvl <= 1024:
            return (4, False, 2, True)
        else:
            return (8, True, 1, False)    # was 16, try 8
    else:  # bs=256
        if kvl <= 1024:
            return (4, True, 1, False)
        else:
            return (16, True, 1, False)   # was 32, try 16
