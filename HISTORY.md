...
|--------|--------|
| **Problem** | mixed-mla |
| **Goal** | Test bf16 KV path and HIP load_inline compilation |
| **Techniques** | 1) Triton bf16 split-K; 2) HIP load_inline with PYTORCH_ROCM_ARCH=gfx950 |
| **Code / commit** | `triton_bf16_splitk.py`, `hip_bf16_mla.py` |
| **Evidence** | Triton bf16: 4/4 correct, 103-19400µs (scalar, no MFMA); HIP bf16: 4/4 correct, 4.3-34ms (scalar) |
| **Popcorn** | `test` ✅ both; `benchmark` ✅ both (but slow) |
| **Result** | **KEY BREAKTHROUGH: HIP load_inline compiles in ~30s with gfx950!** Previous "17 min timeout" was wrong arch. Teammate's mxfp4-mm code proves the pattern works. |
| **What didn't work** | Scalar dot products in both kernels - need MFMA for QK computation |
| **Learnings** | 1) **PYTORCH_ROCM_ARCH=gfx950** is critical for fast compile; 2) bf16 KV passes correctness easily; 3) Teammate's `load_inline` with `--offload-arch=gfx950 -std=c++20 -O3` compiles fine; 4) bf16 MFMA: `__builtin_amdgcn_mfma_f32_16x16x32_bf16` available |
| **Next bet** | Rewrite HIP kernel with bf16 MFMA tiled QK, split-K with online softmax, MFMA V accumulation. Target: match or beat aiter's 68µs |
| **Artifacts** | `triton_bf16_splitk.py`, `hip_bf16_mla.py` |
